...
public class GetSlideShowsByUserResponseHandler extends AbstractResponseHandler {

    private GetSlideShowResponseHandler slideshowHandler;

    private final SlideshowsByUser slideshowsByUser = new SlideshowsByUser();
    ...
    public SlideshowsByUser getSlideshowsByUser() {
        return slideshowsByUser;
    }

    public void handleEndOf(String uri, String localName, String qName)
            throws SAXException {
        if (localName.equals(GetSlideShowResponseHandler.TAG_SLIDESHOW)) {
            slideshowsByUser.getSlideshow().add(slideshowHandler.getSlideshow());
            slideshowHandler = null;
        } else if (localName.equals(GetSlideShowsByGroupResponseHandler.TAG_NAME)) {
            slideshowsByUser.setName(value.trim());
        } else if (localName.equals(GetSlideShowsByGroupResponseHandler.TAG_COUNT)) {
            slideshowsByUser.setCount(parseLong(value));
        } else if (slideshowHandler != null) {
    ...
    public Object getResult() {
        return this.getSlideshowsByUser();
    }
}
